...
**Status:** accepted; session-retention statement superseded in part by ADR-0002
**Date:** 2026-07-31
...
Draft generation is independent of the ASR CPU budget. 2xbrainz uses AIGate as
its sole service boundary. AIGate receives minimized transcript text, never
audio; its own configuration controls any upstream provider and egress policy.

Sessions were initially ephemeral. ADR-0002 supersedes this statement for a
bounded structured reconstruction log. Raw audio remains ephemeral, and export
and deletion commands remain outside this mode.
...
PipeWire system-audio capture target: a monitor source when available, or a
directly capturable sink. It does not claim compatibility with a specific
calling application until that application has passed the live test.
...
- A host without PipeWire-visible microphone and system-audio capture targets
  cannot run `live`, but can use Dockerized replay and diagnostics.
- Encrypted persistence, export, and deletion require a future retention-mode
  decision and separate implementation. ADR-0002 permits only the bounded local
  reconstruction log.
...
PipeWire nodes, then run `make run` with a measured resource budget. See
